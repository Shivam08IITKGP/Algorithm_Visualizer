...
def knapsack_duplicate(weights, values, capacity):
    n = len(values)
...
def visualize_knapsack_duplicate(weights, values, capacity, output_file):
    n = len(weights)
    dp = knapsack_duplicate(weights, values, capacity)  # Compute DP table
    frames = []
    ...
    def update(frame):
        ...
        if i == 0:
            ...
            ax.set_title(f'Duplicate Knapsack DP Table, now at item {i}, capacity {w}')
...
def main():
    ...
    values = [10, 40, 50, 70]
    capacity = 8
    output_file = 'knapsack_duplicate_animation.gif'

    # Visualize the Duplicate Knapsack problem
    dp_table = visualize_knapsack_duplicate(weights, values, capacity, output_file)
    print(f"DP Table:\n {np.array(dp_table)}")
