...
class SegmentTree:
    ...
    def update_range_util(self, node, start, end, l, r, value, queryctr, orig_l = None, orig_r = None):
        self.add_frame(node, f"Updating node {node}, range [{start}, {end}]", f"Query {queryctr}: {orig_l} {orig_r} {value}")
        if self.lazy[node] != 0:
            ...
            self.lazy[node] = 0
            self.add_frame(node, f"Applying lazy value at node {node}, new value {self.tree[node]}", f"Query {queryctr}: {orig_l} {orig_r} {value}")
        ...
        if start >= l and end <= r:
            ...
            self.add_frame(node, f"Updated node {node}, range [{start}, {end}], new value {self.tree[node]}", f"Query {queryctr}: {orig_l} {orig_r} {value}")
            return
        ...
        self.add_frame(node, f"Updated node {node} after recursion, new value {self.tree[node]}", f"Query {queryctr}: {orig_l} {orig_r} {value}")
    ...
    def query_range_util(self, node, start, end, l, r, queryctr, orig_l, orig_r):
        ...
        if self.lazy[node] != 0:
            ...
            if start != end:
                ...
                self.lazy[2 * node + 2] += self.lazy[node]
            self.lazy[node] = 0
            self.add_frame(node, f"Applying lazy value at node {node}, new value {self.tree[node]}", f"Query {queryctr}: {orig_l} {orig_r}")

        if start > end or start > r or end < l:
            return 0

        if start >= l and end <= r:
            self.add_frame(node, f"Returning value from node {node}, range [{start}, {end}], value {self.tree[node]}", f"Query {queryctr}: {orig_l} {orig_r}")
        ...
        self.add_frame(node, f"Returning value from node {node} after recursion, value {left_query + right_query}", f"Query {queryctr}: {orig_l} {orig_r}")
    ...
    def visualize_segment_tree_operations(self, queries=[[2, 5, 5], [2, 4], [1, 4]], output_file='segment_tree_lazy_operations.gif'):
        ...
        anim = animation.ArtistAnimation(fig, [[plt.imshow(frame, aspect='auto')] for frame in self.frames], interval=1500, repeat=False)

        print(f"Saving animation to {output_file}...")
        anim.save(output_file, writer='pillow', dpi=100, savefig_kwargs={'transparent': True, 'facecolor': 'none', 'pad_inches': 0})
