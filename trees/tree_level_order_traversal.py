...
def visualize_level_order_traversal(graph, output_file):
    traversal_order = level_order_traversal(graph)
    G = nx.DiGraph()
    for node in graph:
        G.add_node(node)
        for neighbour in graph[node]:
...
def main():
    ...
    output_file = 'level_order_traversal.gif'
    visualize_level_order_traversal(graph, output_file)
